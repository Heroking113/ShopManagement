$(function() {
	...
	// 页码
	var pageNum = 1;
	// 从地址栏 URL里尝试获取parent shop category id.
				...
				.getJSON(
						...
						function(data) {
							if (data.success) {
								...
								var html = '';
								html += '<a href="#" class="button" style="width:98%" data-category-id="">全部类别</a>';
								// 遍历店铺类别列表，拼接出a标签集
								shopCategoryList
										.map(function(item, index) {
											html += '<a href="#" class="button" style="width:98%" data-category-id='
													+ item.shopCategoryId
													+ '>'
			...
			function(e) {
				...
				if (parentId) {
					...
					if ($(e.target).hasClass('button-fill')) {
						...
						$(e.target).addClass('button-fill').siblings().remove(
								'button-fill');
					}
	...
	// 需要查询的店铺名字发生变化后，重置页码，清空原先的店铺列表，按照新的名字 去查询
