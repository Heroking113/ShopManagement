$(function() {
				...
				.getJSON(
						...
						function(data) {
							if (data.success) {
								...
								// 从返回的JSON当中获取到product对象的信息，并赋值给表单
								...
								optionArr
										.map(function(item, index) {
											var isSelect = optionSelected === item.porductCategoryId ? 'selected'
													: '';
	...
	$('#submit').click(
			function() {
				...
				// 获取缩略图文件流:这里和shopoperation.js写的一样，但是shop可以上传文件到后台，product不行，这个部分暂时先放着，有时间了再来解决
				var thumbnail = $('#small-img')[0].files[0];
				formData.append('thumbnail', thumbnail);
				// 遍历商品详情图控件，获取里面的文件流
				$('.detail-img').map(
						function(index, item) { // 判断该控件是否已选择了文件 if
							($('.detail-img')[index].files.length > 0)
							{
				...
				// 将product json对象转成字符流保存在表单对象key为productStr的键值对里
